...
object consumer {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val schema = new StructType()
      ...
      .add("students", IntegerType)          // ✅ هذا هو العمود الصحيح
    ...
    val query = parsed.writeStream
      .foreachBatch { (batchDF: DataFrame, batchId: Long) =>
        ...
        batchDF.filter(col("source_type") === "professors")
          ...
          .write.format("mongo")
          .option("uri", "mongodb://127.0.0.1/StreamRoom.professors")
          .mode("append").save()

        batchDF.filter(col("source_type") === "sections")
          .dropDuplicates("section_id")
          .write.format("mongo")
          .option("uri", "mongodb://127.0.0.1/StreamRoom.sections")
          .mode("append").save()
